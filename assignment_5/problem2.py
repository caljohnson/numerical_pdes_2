...
def high_res_method(u0, flux, delT, delX,Tf):
	...
	for t in range(1,Nt+1):
		...
		left_ghosts = np.r_[u_old[-1],u_old[-2]]
		right_ghosts = np.r_[u_old[1],u_old[0]]
...
if __name__ == '__main__':
	...
	if IC ==0:
		...
		# u5 = np.asarray([cos(16*pi*(x-5*a))*exp(-50*((x-5*a)-0.5)**2) for x in X])
	if IC == 1:
		u0 = np.asarray([sin(2*pi*x)*sin(4*pi*x) for x in X])
		# u5 = np.asarray([sin(2*pi*(x-5*a))*sin(4*pi*(x-5*a)) for x in X])
	if IC==2:
		u0 = np.r_[np.zeros(int(Nx/4)), np.ones(int(Nx/2)), np.zeros(int(Nx/4))]	
		# u5 = np.r_[np.zeros(int(Nx/4)), np.ones(int(Nx/2)), np.zeros(int(Nx/4)+1)]
